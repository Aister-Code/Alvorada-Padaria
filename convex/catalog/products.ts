...
import { assertCategoryExists, assertUniqueDocumentKey, findByDocumentKey } from "./dbHelpers";
import {
...
async function listActiveOptions(ctx: { db: DatabaseReader }, productId: Id<"products">) {
  const options = await ctx.db
    .query("productOptions")
    .withIndex("by_product_active_order", (q) => q.eq("productId", productId).eq("active", true))
    .collect();
  return options.sort(compareCatalogOrder);
}

async function buildPublicProduct(ctx: { db: DatabaseReader }, product: Doc<"products"> | null) {
  ...
  const sellability = determineProductSellability({ product, category, options });
...
export const listPublicByCategoryMaster = query({
  ...
  handler: async (ctx, args) => {
    const products = await ctx.db
      .query("products")
      .withIndex("by_category_active_order", (q) => q.eq("categoryId", args.categoryId).eq("active", true))
      .collect();
    const mapped = await Promise.all(products.sort(compareCatalogOrder).map((product) => buildPublicProduct(ctx, product)));
...
export const listSellable = query({
  ...
  handler: async (ctx) => {
    ...
    const mapped = await Promise.all(products.map((product) => buildPublicProduct(ctx, product)));
...
async function assertDocumentalIdAvailable(ctx: { db: DatabaseReader }, documentalId: string | undefined, currentId?: string) {
  if (!documentalId) return;
  const existing = await ctx.db
    .query("products")
    .withIndex("by_documental_id", (q) => q.eq("documentalId", documentalId))
    .first();
  if (existing && existing._id !== currentId) throw new ConvexError("documentalId_duplicado");
}
...
async function assertSlugAvailable(ctx: { db: DatabaseReader }, slug: string, currentId?: string) {
  ...
  if (existing && existing._id !== currentId) throw new ConvexError("slug_duplicado");
}
...
export const createMaster = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    assertProductPriceInvariants({ basePrice: args.basePrice, priceStatus: args.priceStatus });
    if (args.active ?? false) {
      const sellability = determineProductSellability({
        product: { active: true, basePrice: args.basePrice, priceStatus: args.priceStatus },
        category,
        options: [],
      });
...
export const updateMaster = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    await ctx.db.patch(args.productId, {
      ...(args.documentalId !== undefined ? { documentalId: args.documentalId } : {}),
      ...
      ...(args.shortDescription !== undefined ? { shortDescription: args.shortDescription, description: args.shortDescription } : {}),
      ...
      ...(args.productionSector !== undefined ? { productionSector: args.productionSector } : {}),
...
export const setActiveMaster = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (args.active) {
      ...
      const sellability = determineProductSellability({ product: { ...product, active: true }, category, options });
      if (!sellability.sellable) throw new ConvexError(sellability.reason);
    }
    await ctx.db.patch(args.productId, {
      active: args.active,
